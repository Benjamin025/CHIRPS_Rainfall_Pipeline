...
class CHIRPSCompleteWorkflow:
    ...
    def __init__(self, base_dir=None, version="3.0", product_type="final"):
        """
        ...
        version : str
            CHIRPS version: "2.0" or "3.0" (default: "3.0" - latest)
        product_type : str
        ...
        if base_dir is None:
            self.base_dir = Path.home() / "Documents" / "Benjamin" / "CHIRPS" / f"CHIRPS_v{version}_{product_type}"
        else:
            self.base_dir = Path(base_dir)
        
        # Create directory structure
        self.dirs = {
            'raw_gz': self.base_dir / "raw_gz",        # Original .gz files (optional)
        ...
        if version == "2.0":
            ...
            coverage = "60°S to 60°N (global land)"
        
        print(f"\n🌧️ CHIRPS v{version} {'PRELIMINARY' if product_type == 'prelim' else 'FINAL'} WORKFLOW")
        print(f"📁 Base directory: {self.base_dir}")
        print(f"🌍 Coverage: {coverage}")
        print(f"📏 Resolution: 0.05° (~5.5 km)")
        print(f"📊 Units: mm/month (no conversion needed)")
        if version == "3.0":
            print(f"🆕 Version 3.0: 4x more stations, improved variance, gauge-undercatch corrected")
        print("="*70)
    ...
    def download_single_file(self, year, month, max_retries=3):
        ...
        # Check if file is gzipped or direct tif
        ...
        if is_gzipped:
            ...
            # v3.0 format - direct tif
            source_path = year_tif_dir / filename
        ...
        # For gzipped files, check if .gz exists but not extracted
        if is_gzipped and source_path.exists() and not tif_path.exists():
    ...
    def _create_download_log(self, downloaded_files, failed_months):
        ...
        with open(log_file, 'w') as f:
            f.write("="*70 + "\n")
            f.write(f"CHIRPS v{self.version} ({self.product_type.upper()}) DOWNLOAD LOG\n")
            f.write("="*70 + "\n\n")
...
def main():
    ...
    # Product type selection (only for v3.0)
    product_type = "final"
    if version == "3.0":
        print("\nSelect product type:")
        print("1. Final (best quality, ~3rd week of following month)")
        print("2. Preliminary (rapid updates, 2 days after pentad)")
        
        product_choice = input("\nProduct (1-2) [default: 1]: ").strip()
        product_type = "prelim" if product_choice == "2" else "final"
    
    # Create workflow instance
    workflow = CHIRPSCompleteWorkflow(version=version, product_type=product_type)
    ...
    while True:
        ...
        if choice == '1':
            ...
            if version == "2.0":
                size_estimate = total_months * 30 / 1024  # ~30MB compressed
            else:
                size_estimate = total_months * 50 / 1024  # ~50MB uncompressed
...
def quick_start():
    ...
    print("="*70)
    print("Testing with 2023-2024 (2 years) using CHIRPS v3.0")
    print("="*70)
