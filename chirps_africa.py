...
- Coverage: Africa continent (pre-clipped by CHC — no post-processing needed)
...
class CHIRPSCompleteWorkflow:
    """Complete CHIRPS Africa workflow: download, organize, validate"""
    
    def __init__(self, base_dir=None, version="2.0", product_type="final"):
        """
        Initialize CHIRPS Africa workflow
        ...
        version : str
            CHIRPS version: "2.0" (Africa endpoint is v2.0)
        product_type : str
        ...
        # -----------------------------------------------------------------
        # Africa-specific URL — files are pre-clipped to Africa by CHC,
        # no post-download clipping required.
        # -----------------------------------------------------------------
        self.BASE_URL = "https://data.chc.ucsb.edu/products/CHIRPS-2.0/africa_monthly/tifs"
        
        # Base directory structure
        if base_dir is None:
            self.base_dir = Path.home() / "Documents" / "Benjamin" / "CHIRPS" / "CHIRPS_Africa"
        else:
        ...
        self.dirs = {
            ...
            'geotiffs': self.base_dir / "geotiffs",   # Extracted .tif files  ← MAIN OUTPUT
        ...
        print(f"\n🌍 CHIRPS v{version} {'PRELIMINARY' if product_type == 'prelim' else 'FINAL'} — AFRICA WORKFLOW")
        print(f"📁 Base directory: {self.base_dir}")
        print(f"🔗 Source URL    : {self.BASE_URL}")
        print(f"🌍 Coverage      : Africa continent (pre-clipped by CHC)")
    ...
    def download_single_file(self, year, month, max_retries=3):
        """
        Download a single monthly CHIRPS Africa file
        
        Returns:
        --------
        tuple: (source_path, tif_path) or (None, None) if failed
        """
        ...
        if tif_path.exists():
            ...
            return source_path, tif_path
        
        # Check if .gz exists but not extracted
        if source_path.exists() and not tif_path.exists():
            print(f"  📦 Extracting existing .gz file...")
            success = self._extract_gz_file(source_path, tif_path)
            if success:
                return source_path, tif_path
        ...
        for attempt in range(max_retries):
            try:
                ...
                temp_path = source_path.with_suffix('.downloading')
    ...
    def download_year_range(self, start_year=1981, end_year=2027):
        ...
        print(f"\n📥 DOWNLOADING AFRICA DATA: {start_year} to {end_year}")
        print("="*70)
        ...
        if failed_months:
            print("\nFailed months:")
            for year, month in failed_months[:20]:
    ...
    def _create_download_log(self, downloaded_files, failed_months):
        ...
        with open(log_file, 'w') as f:
            f.write("="*70 + "\n")
            f.write(f"CHIRPS v{self.version} ({self.product_type.upper()}) AFRICA DOWNLOAD LOG\n")
            f.write("="*70 + "\n\n")
            ...
            f.write("-"*50 + "\n")
            for year, month, source_path, tif_path in downloaded_files:
    ...
    def _create_metadata(self, metadata_path, tif_file, year, month, crs, bounds, shape, stats):
        ...
        try:
            ...
            with open(metadata_path, 'w') as f:
                f.write("="*60 + "\n")
                f.write(f"CHIRPS v{self.version} MONTHLY METADATA — AFRICA\n")
                f.write("="*60 + "\n\n")
                ...
                f.write(f"  Dataset: CHIRPS v{self.version} — Africa\n")
                f.write( "  Resolution: 0.05° (~5.5 km)\n")
                f.write( "  Coverage: Africa continent (pre-clipped by CHC)\n")
                f.write( "  Units: mm/month (no conversion needed)\n")
                f.write( "  Source: Climate Hazards Center, UCSB\n")
                f.write( "  URL: https://data.chc.ucsb.edu/products/CHIRPS-2.0/africa_monthly/tifs\n")
    ...
    def _create_preview(self, data, bounds, output_path, year, month, nodata=None):
        ...
        try:
            ...
            ax.set_title(f'CHIRPS v{self.version} Africa - {month_name} {year}', 
    ...
    def _create_validation_log(self, validated_files, failed_files):
        ...
        with open(log_file, 'w') as f:
            f.write("="*70 + "\n")
            f.write(f"CHIRPS v{self.version} AFRICA VALIDATION LOG\n")
            f.write("="*70 + "\n\n")
    ...
    def run_complete_workflow(self, start_year=1981, end_year=2027, skip_download=False):
        ...
        print("\n" + "="*70)
        print(f"🚀 CHIRPS v{self.version} AFRICA COMPLETE WORKFLOW")
        print("="*70)
        ...
        print(f"\n💡 DATA NOTES:")
        print(f"  • Files downloaded from Africa-specific CHC endpoint")
        print(f"  • Pre-clipped to Africa — no extra processing needed")
        print(f"  • CHIRPS data is ALREADY in mm/month (no conversion needed)")
        print(f"  • Files are .tif.gz (auto-extracted to .tif)")
        print(f"  • Resolution: 0.05° (~5.5 km)")
...
def main():
    ...
    print("\n" + "="*70)
    print("🌍 CHIRPS AFRICA WORKFLOW MANAGER")
    print("="*70)
    print("Downloads CHIRPS Africa data (1981-2027) as monthly GeoTIFFs")
    print("Source: https://data.chc.ucsb.edu/products/CHIRPS-2.0/africa_monthly/tifs/")
    print("Resolution: 0.05° | Units: mm/month")
    print("="*70)
    
    # Africa endpoint is always v2.0
    workflow = CHIRPSCompleteWorkflow(version="2.0", product_type="final")
    ...
    while True:
        ...
        if choice == '1':
            ...
            size_estimate = total_months * 3 / 1024  # ~3 MB per Africa file (much smaller than global)
            
            print(f"\n⚠️ This will process up to {total_months} months")
            print(f"   Approximate total download size: {size_estimate:.1f} GB")
...
def quick_start():
    ...
    print("\n" + "="*70)
    print("🚀 CHIRPS AFRICA WORKFLOW QUICK START")
    print("="*70)
    print("Testing with 2023-2024 (2 years) — Africa monthly data")
    print("="*70)
    
    workflow = CHIRPSCompleteWorkflow(version="2.0", product_type="final")
    workflow.run_complete_workflow(start_year=2023, end_year=2024)
...
if __name__ == "__main__":
    ...
    print("2. Quick start (test with 2023-2024)")
